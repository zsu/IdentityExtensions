...
{
    public class PermissionStore<TUser, TUserStore>
    : PermissionStore<string, ExtendedRole, TUser, TUserStore, IdentityUserLogin, IdentityRolePermission, IdentityUserClaim, IdentityUserRole, IdentityPermission>
    where TUser : IdentityUser, new()
    where TUserStore : UserStore<TUser, ExtendedRole, string, IdentityUserLogin, IdentityUserRole, IdentityUserClaim>
    {
        public PermissionStore(ExtendedDbContext<TUser> context) : base(context)
        {
        }
    }
    /// <summary>
    ...
    where TRole : ExtendedRole<TKey, TUserRole, TRolePermission>, new()
    where TKey : IEquatable<TKey>
    {
        ...
        private readonly IDbSet<TRolePermission> _rolePermissions;

        /// <summary>Context for the store</summary>
        public ExtendedDbContext<TUser, TRole, TKey, TUserLogin, TUserRole, TPermission, TRolePermission, TUserClaim>
            Context
        { get; private set; }
        ...
        public PermissionStore(
            ExtendedDbContext<TUser, TRole, TKey, TUserLogin, TUserRole, TPermission, TRolePermission, TUserClaim>
                context)
        {
            if (context == null)
            ...
            _userRoles = (IDbSet<TUserRole>)this.Context.Set<TUserRole>();
            _rolePermissions = (IDbSet<TRolePermission>)this.Context.Set<TRolePermission>();
        }
        ...
        public virtual async Task InitialConfiguration()
        {
            ...
            //    this._roles.Add(new TRole { Name = nameof(Roles.Administrator), Description = "Super Administrator" });
            ...
            //catch (Exception ex)
            //{
            //    var ex1 = ((System.Data.Entity.Validation.DbEntityValidationException)ex).EntityValidationErrors;
            //}
        }
        ...
        //    });
        //}
        public virtual async Task<bool> AuthorizePermissionAsync(List<string> roles)
        {
            ...
            return await AuthorizePermissionAsync(userName, roles);
        }
        public virtual async Task<bool> AuthorizePermissionAsync(string userName, List<string> roles)
        {
            //if (!Thread.CurrentPrincipal.Identity.IsAuthenticated)
            //    return await Task.FromResult(false);

            //var name = Thread.CurrentPrincipal?.Identity?.GetUserName();
            if (string.IsNullOrEmpty(userName))
                throw new ArgumentNullException(nameof(userName));
            var query = (from ur in _userRoles
                         join u in _user on ur.UserId equals u.Id
                         join r in _roles on ur.RoleId equals r.Id
                         join rp in (from rp1 in _rolePermissions join p in _permissions on rp1.PermissionId equals p.Id select new { rp1.RoleId, rp1.PermissionId, p.Name }) on ur.RoleId equals rp.RoleId into rp2
                         from lrp in rp2.DefaultIfEmpty()
                         where u.UserName == userName && (roles.Contains(r.Name) ||
                         (lrp != null && roles.Contains(lrp.Name)))
                         select ur);
